...
function parseImage(buffer, mime) {
  ...
  return {
    html: `<figure><img src="data:${mime};base64,${b64}" style="max-width:100%"/><figcaption>Зураг — тайлбар/текстээ энд бичнэ үү</figcaption></figure>`,
    warnings: [],
  };
}
...
export async function parseFile({ buffer, originalname, mimetype }) {
  ...
  switch (kind) {
    ...
    case 'image':
      return finish(parseImage(buffer, mimetype?.startsWith('image/') ? mimetype : 'image/png'));
    case 'text':
