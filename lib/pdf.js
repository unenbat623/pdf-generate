...
// Хэрэглэгчийн сонгож болох форматууд (өргөн×өндөр, мм)
...
// Захын урьдчилсан тохиргоо (мм)
export const MARGIN_PRESETS = {
  normal: { top: 14, right: 12, bottom: 14, left: 12 },
  narrow: { top: 6, right: 6, bottom: 6, left: 6 },
  wide: { top: 22, right: 20, bottom: 22, left: 20 },
};
...
async function renderPdf(html, opts = {}) {
  // Форматын тохиргоо (хэрэглэгчийн сонголт эсвэл үндсэн утга)
  const format = PAGE_SIZES_MM[opts.format] ? opts.format : 'A4';
  const [pageWMM, pageHMM] = PAGE_SIZES_MM[format];
  const margin = MARGIN_PRESETS[opts.margin] || MARGIN_PRESETS.normal;
  const PRINTABLE_PX = {
    portrait: (pageWMM - margin.left - margin.right) * MM_TO_PX,
    landscape: (pageHMM - margin.left - margin.right) * MM_TO_PX,
  };
...
<style>${PRINT_CSS}\n${css}</style></head><body class="doc-body">${safeHtml}</body></html>`;
  ...
  try {
    ...
    let scale = remaining > printable ? Math.max(0.35, Math.min(1, printable / remaining)) : 1;
    // Хэрэглэгчийн сонгосон масштаб (үсгийн хэмжээ) — автоматаар тооцсон дээр үржинэ
    if (opts.userScale) scale = Math.max(0.1, Math.min(2, scale * opts.userScale));
    console.log(
      `pdf: агуулга ${contentWidth}px → ${landscape ? 'хэвтээ' : 'босоо'} ${format}, ${zoomed} блок багтаалаа, ерөнхий масштаб ${scale.toFixed(2)}`
    );
