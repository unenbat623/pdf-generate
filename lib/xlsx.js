...
function renderSheet({ name, meta, sjsSheet, styles, anchors, sheetRels }) {
  ...
  // Сүүл талын хоосон мөр/баганыг тайрна. Excel-ийн "used range" нь зөвхөн
  // загвартай (утгагүй) нүднүүдээс болж хэдэн мянган мөр болтлоо томордог —
  // тэр чигээр нь зурвал PDF хэдэн зуун хоосон хуудастай болж, Chromium
  // "Printing failed" гээд унадаг.
  let lastRow = -1;
  let lastCol = -1;
  ...
  for (const m of meta.merges) {
    lastRow = Math.max(lastRow, Math.min(m.e.row, maxRow));
    lastCol = Math.max(lastCol, Math.min(m.e.col, maxCol));
  }
  for (const a of anchors) {
    if (a.from) {
      lastRow = Math.max(lastRow, Math.min(a.to ? a.to.row : a.from.row, maxRow));
      lastCol = Math.max(lastCol, Math.min(a.to ? a.to.col : a.from.col, maxCol));
    } else if (a.absPos) {
      // Пикселээр байрладаг зураг — хүрээг хэвээр үлдээж найдвартай байлгана
      lastRow = maxRow;
      lastCol = maxCol;
    }
  }
  ...
  maxCol = Math.min(maxCol, Math.max(lastCol, 0));
